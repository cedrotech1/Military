...
export const processAddUsers = async (req, res) => {
  ...
  const filePath = file.tempFilePath || file.path; // Get correct file path
  ...
  try {
    ...
    for (const row of data) {
        ...
        const missingFields = Object.entries(user)
            .filter(([key, value]) => !value || (isNaN(value) && ['departmentId', 'armyid'].includes(key)))
            .map(([key]) => key);
        ...
        if (await getUserByPhone(user.phone)) {
            console.log(`Duplicate Phone: ${user.phone}`);
            duplicateUsers.push(`Phone: ${user.phone}`);
            continue;
        }

        if (await getUserByAID(String(user.armyid))) {
            console.log(`Duplicate Army ID: ${user.armyid}`);
...
export const uploadExcel = (req, res) => {
  ...
  const file = req.files.file;
  const filePath = file.tempFilePath || file.path; // Get file path
